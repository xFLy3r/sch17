	...
	for ($currentDay = $startingDay; $currentDay <= $endingDay; $currentDay++) {
		...
		for ($i = 0; $i <= 8; $i++) {
			...
			if ($i == 0) echo "<tr><td class='date' colspan='3'>".$homework[1]."</td></tr>";
				...
				$explodeLesson = explode("<^>", $lesson[$i + 1]);
				$less[0] = less($explodeLesson[0], $currentDay, $i);
				...
				if (count($explodeLesson) == 2) {
					$less[1] = less($explodeLesson[1], $currentDay, $i, 1);
...
function less($lesson, $day, $i, $secondGroup=0) {
	...
	while($row = mysqli_fetch_row($result)) {
		...
		if ($lesson == $row[$secondGroup]) $str .= '<option onClick="blurt(this)" selected>'.$row[$secondGroup].'</option>';
		else $str .= '<option onClick="blurt(this)">'.$row[$secondGroup].'</option>';
	}
	$str .= "</select></div>";
	return $str;
}
